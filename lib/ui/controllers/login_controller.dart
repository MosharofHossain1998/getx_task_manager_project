import 'package:get/get.dart';
import '../../data_network_caller/models/user_model.dart';
import '../../data_network_caller/network_caller.dart';
import '../../data_network_caller/network_response.dart';
import '../../data_network_caller/utility/url.dart';
import 'authentication_controller.dart';

class LoginController extends GetxController{

  bool  _loginInProgress = false;
  String _failedMassage = '';

  bool get loginInprogress => _loginInProgress;
  String get failerMassage => _failedMassage;

  Future<bool> login(String email, String password) async {

    _loginInProgress = true;
    update();
    NetworkResponse response =
    await NetworkCaller().postRequest(Urls.login, requestBody: {
      "email": email,
      "password": password,
    },isLogin: true);
    _loginInProgress = false;
    update();
    if (response.isSuccess) {
      await Get.find<AuthenticationController>().saveUserInformation(
        response.jsonResponse['token'],
        UserModel.fromJson(response.jsonResponse['data']),
      );
      return true;
    } else {
      if (response.statusCode == 401) {
        _failedMassage = "Invalid Email Or Password, please check";

      } else {
        _failedMassage = 'Login Faield! , Please Try Again';
      }
    }
    return false;
  }
}